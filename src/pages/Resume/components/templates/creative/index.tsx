...
const CreativeTemplate: React.FC<Props> = ({ data, onSectionClick, activeSection }) => {
  ...
  const basicSections = enabled.filter(s => s.id === 'basic')
  const otherSections = enabled.filter(s => s.id !== 'basic')

  return (
    <div>
      {basicSections.map(s => renderSection(s.id))}
      <div style={{ padding: `0 ${globalSettings.pagePadding || 20}px ${globalSettings.pagePadding || 20}px` }}>
        {otherSections.map(s => renderSection(s.id))}
      </div>
    </div>
  )
}
