...
const ProjectSection: React.FC<Props> = ({ projects, globalSettings, sectionTitle = '项目经历' }) => {
  ...
  return (
    <div>
      ...
      {visible.map(proj => (
        <div key={proj.id} style={{ marginTop: g?.paragraphSpacing || 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ fontWeight: 700, fontSize: (g?.headerSize || 15) + 'px' }}>{proj.name}</span>
            <span style={{ fontSize: (g?.baseFontSize || 13) + 'px', color: '#888' }}>{proj.date}</span>
          </div>
          {proj.role && <div style={{ fontSize: (g?.baseFontSize || 13) + 'px', color: '#555', marginTop: 2 }}>{proj.role}</div>}
